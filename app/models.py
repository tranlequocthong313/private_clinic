from sqlalchemy import Integer, Column, ForeignKey, DateTime, Double, Boolean, String, Date, Enum
...
class User(db.Model):
    ...
    appointment_schedule = relationship('AppointmentSchedule', backref='nurse', lazy=True, foreign_keys='AppointmentSchedule.nurse_id')
    created_medical_examinations = relationship('MedicalExamination', backref='doctor', lazy=True, foreign_keys='MedicalExamination.doctor_id')
    own_medical_examinations = relationship('MedicalExamination', backref='patient', lazy=True, foreign_keys='MedicalExamination.patient_id')
    created_bills = relationship('Bill', backref='cashier', lazy=True, foreign_keys='Bill.cashier_id')
    own_bills = relationship('Bill', backref='patient', lazy=True, foreign_keys='Bill.patient_id')
    medical_registrations = relationship('MedicalRegistration', backref='patient', lazy=True)
...
class MedicalRegistration(db.Model):
    ...
    fulfilled = Column(Boolean, default=False)
    patient_id = Column(Integer, ForeignKey(User.id), nullable=False)
    appointment_schedule_id = Column(Integer, ForeignKey(AppointmentSchedule.id), nullable=True)

    def __str__(self):
        return self.id


medicine_type = db.Table('medicine_medicine_type',
    db.Column('id', db.Integer, primary_key=True, autoincrement=True),
